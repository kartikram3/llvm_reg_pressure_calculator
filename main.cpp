...
int main(int argc, char ** argv){
  ...
  if(argc != 2){ std :: cout << "Please specify *.ll file as argument "
                             << std::endl;
                 exit(0);
               }

  llvm_module_ =
    llvm::parseIRFile(argv[1], llvm_err_, llvm_context_);
 ...
 flow_graph * fg = new flow_graph( fita, end ); //calculates reg pressur
